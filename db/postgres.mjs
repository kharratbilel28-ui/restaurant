...
const schema = [
  ...
  `CREATE TABLE IF NOT EXISTS restaurant_device_sessions (token_hash text PRIMARY KEY, restaurant_id text NOT NULL REFERENCES restaurants(id) ON DELETE CASCADE, expires_at timestamptz NOT NULL, created_at timestamptz NOT NULL DEFAULT now())`,
...
export async function initPostgres(fallbackState) {
  ...
  try {
    ...
    if (state.floorPlan?.fileName) await client.query('INSERT INTO floor_plan_configs (restaurant_id, file_name, pdf_data, positions) VALUES ($1, $2, $3, $4::jsonb)', ['restaurant-demo', state.floorPlan.fileName, '', JSON.stringify(state.floorPlan.positions || {})])
...
export async function createRestaurantDeviceSession(tokenHash, restaurantId, expiresAt) {
  await pool.query('INSERT INTO restaurant_device_sessions (token_hash, restaurant_id, expires_at) VALUES ($1, $2, $3)', [tokenHash, restaurantId, expiresAt])
}

export async function findRestaurantDeviceSession(tokenHash) {
  const result = await pool.query('SELECT s.restaurant_id AS id, r.identifier, r.name, r.subscription_status AS status, r.subscription_expires_at AS "subscriptionExpiresAt", s.expires_at AS "sessionExpiresAt" FROM restaurant_device_sessions s JOIN restaurants r ON r.id = s.restaurant_id WHERE s.token_hash = $1 AND s.expires_at > now()', [tokenHash])
  return result.rows[0] || null
}

export async function deleteRestaurantDeviceSession(tokenHash) {
  await pool.query('DELETE FROM restaurant_device_sessions WHERE token_hash = $1', [tokenHash])
}

export async function deleteRestaurantDeviceSessions(restaurantId) {
  await pool.query('DELETE FROM restaurant_device_sessions WHERE restaurant_id = $1', [restaurantId])
}
...
export async function readPostgres(restaurantId) {
  ...
  try {
    const [reservations, users, orders, tables, inventory, menu, withdrawals, floorPlan, receipts] = await Promise.all([
      ...
      client.query('SELECT file_name AS "fileName", \'\'::text AS "pdfData", positions FROM floor_plan_configs WHERE restaurant_id = $1', [restaurantId]),
...
export async function savePostgres(restaurantId, state) {
  ...
  try {
    ...
    if (state.tables.length) await client.query('DELETE FROM dining_tables WHERE restaurant_id = $1 AND NOT (id = ANY($2::text[]))', [restaurantId, state.tables.map((table) => table.id)])
    ...
    if (state.floorPlan?.fileName) await client.query('INSERT INTO floor_plan_configs (restaurant_id, file_name, pdf_data, positions) VALUES ($1, $2, $3, $4::jsonb) ON CONFLICT (restaurant_id) DO UPDATE SET file_name = EXCLUDED.file_name, pdf_data = EXCLUDED.pdf_data, positions = EXCLUDED.positions, updated_at = now()', [restaurantId, state.floorPlan.fileName, '', JSON.stringify(state.floorPlan.positions || {})])
