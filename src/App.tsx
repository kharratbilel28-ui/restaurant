...
import './App.css'
import './modules.css'
import { createInvoice, createMenuItem, createOrder, createReservation, createStockReceiptDraft, createStockWithdrawal, getDashboard, getFloorPlan, getHealth, getInventory, getMenu, getSession, getStockReceipts, getStockWithdrawals, login, logout, openCashDrawer, reviewStockReceipt, saveFloorPlan, sendInvoiceEmail, updateInventory, updateMenuItem, updateOrderStatus, type Dashboard, type FloorPlanConfig, type InventoryItem, type Invoice, type MenuItem, type Order, type OrderLine, type RestaurantTable, type Role, type SessionUser, type StockReceipt, type StockReceiptLine, type StockWithdrawal } from './api'
...
function App() {
  ...
  const renderView = () => {
    ...
    return <ModuleView title="Paramètres & configuration" description="Gérez les plats, les photos et le plan de salle de l’établissement." icon="⚙"><SettingsModule user={sessionUser} onLogout={handleLogout} menu={menu} onCreateMenuItem={handleMenuItem} onUpdateMenuItem={handleMenuUpdate} tables={tables} floorPlan={floorPlan} onSaveFloorPlan={handleFloorPlanSave} storageMode={storageMode} /></ModuleView>
  }
...
function SettingsModule({ user, onLogout, menu, onCreateMenuItem, onUpdateMenuItem, tables, floorPlan, onSaveFloorPlan, storageMode }: { user: SessionUser | null; onLogout: () => void; menu: MenuItem[]; onCreateMenuItem: (item: { name: string; price: number; image: string }) => void; onUpdateMenuItem: (id: string, item: { name: string; price: number; image: string; active: boolean }) => void; tables: RestaurantTable[]; floorPlan: FloorPlanConfig | null; onSaveFloorPlan: (plan: FloorPlanConfig) => Promise<void>; storageMode: 'postgresql' | 'local-json' | 'unknown' }) { return <div className="settings-stack"><section className="panel settings-module"><div><strong>Session active</strong><span>{user?.name} · {user?.role}</span></div><div className="storage-status"><strong>Base partagée</strong><span>{storageMode === 'postgresql' ? 'PostgreSQL Render' : storageMode === 'local-json' ? 'JSON local à cet ordinateur' : 'État de la connexion inconnu'}</span></div><button className="action-button" onClick={onLogout}>Fermer la session</button></section><MenuConfiguration items={menu} onCreate={onCreateMenuItem} onUpdate={onUpdateMenuItem} /><FloorPlanConfiguration tables={tables} floorPlan={floorPlan} onSave={onSaveFloorPlan} /></div> }
